...
class AdminController extends Controller
{
    ...
    // -------------------------------------------------------------------------
    // CRUD de backups — $request eliminado donde no se usa (S1172)
    // -------------------------------------------------------------------------
    ...
    // =========================================================================
    // Motor automático — $request eliminado (S1172)
    // =========================================================================
    ...
    public function runAutoBackup()
    {
        ...
        return response("✅ BACKUP GENERADO ({$frecuencia}).", 200)
            ->header('Cache-Control', 'no-store, no-cache, must-revalidate, max-age=0');
    }
    ...
    // checkPeriodico dividido en 3 métodos privados para bajar los returns (S1142)
    private function checkPeriodico(string $frecuencia, string $horaDeseada, string $lastRun, Carbon $ahora): array
    {
        ...
        if ($frecuencia === 'diario') {
            return $this->checkDiario($ultimaVezCarbon, $horaActualStr, $horaDeseada, $ahora);
        }
        if ($frecuencia === 'semanal') {
            return $this->checkSemanal($ultimaVezCarbon, $horaActualStr, $horaDeseada, $ahora);
        }
        if ($frecuencia === 'mensual') {
            return $this->checkMensual($ultimaVezCarbon, $horaActualStr, $horaDeseada, $ahora);
        }
    ...
    // =========================================================================
    // Restauración — métodos divididos para bajar returns (S1142)
    // =========================================================================
    ...
    public function restore(Request $request)
    {
        ...
        return $this->ejecutarRestauracion(
            Storage::disk(self::BACKUP_DISK)->get($path),
            $userId,
