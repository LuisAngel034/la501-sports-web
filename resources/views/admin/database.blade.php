    ...
    @if(session('success'))
        <div class="mb-6 p-4 bg-green-500/10 border border-green-500/20 rounded-xl flex flex-col sm:flex-row items-center justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <span class="text-green-600 dark:text-green-400 font-bold text-sm">{{ session('success') }}</span>
            </div>
        </div>
    @endif
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="bg-white dark:bg-[#111] border border-zinc-200 dark:border-white/10 rounded-2xl p-6 shadow-sm">
            ...
            <p class="text-sm text-zinc-500 dark:text-zinc-400 mb-6">
                Genera un respaldo manual de toda la información actual. El archivo se guardará en el historial.
            </p>
        ...
        <div class="bg-white dark:bg-[#111] border border-zinc-200 dark:border-white/10 rounded-2xl p-6 shadow-sm"
            ...
            <form action="{{ route('admin.database.saveAuto') }}" method="POST">
                ...
                <p class="text-sm text-zinc-500 dark:text-zinc-400 mb-6">El sistema guardará copias de seguridad automáticamente según tus reglas.</p>

                <div x-show="autoEnabled" x-transition.opacity class="space-y-4">

                    <div>
                        <label for="frecuencia" class="block text-xs font-bold text-zinc-500 uppercase mb-1">Frecuencia</label>
                        <select id="frecuencia" name="frecuencia" x-model="frecuencia" class="w-full bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-800 dark:text-white outline-none focus:border-green-500">
                    ...
                    <div x-show="frecuencia !== 'intervalo'">
                        <label for="hora" class="block text-xs font-bold text-zinc-500 uppercase mb-1">Hora de ejecución</label>
                        <input type="time" id="hora" name="hora" value="{{ \App\Models\Setting::where('key', 'backup_hora')->value('value') ?? '03:00' }}" class="w-full bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-800 dark:text-white outline-none focus:border-green-500">
                        <p class="text-[11px] text-zinc-500 mt-1">El respaldo se creará en el primer chequeo del servidor después de esta hora.</p>
                    </div>

                    <div x-show="frecuencia === 'intervalo'">
                        <label for="intervalo" class="block text-xs font-bold text-zinc-500 uppercase mb-1">Cada cuánto tiempo</label>
                        <select id="intervalo" name="intervalo" x-model="intervalo" class="w-full bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-800 dark:text-white outline-none focus:border-green-500">
                    ...
                    <div class="flex items-start gap-2 pt-2 pb-2">
                        ...
                        <p class="text-[12px] text-zinc-500 leading-tight">
                            Los respaldos antiguos se eliminarán automáticamente pasadas las 72 horas.
                        </p>
                    </div>
        ...
        <div class="bg-white dark:bg-[#111] border border-zinc-200 dark:border-white/10 rounded-2xl p-6 shadow-sm" x-data="{ tab: 'historial' }">
            ...
            <div x-show="tab === 'historial'" x-transition.opacity style="display: none;">
                @if(count($backups) > 0)
                    ...
                    <div class="mt-4 text-center border-t border-zinc-200 dark:border-white/10 pt-4">
                        <a href="{{ route('admin.database.history') }}" class="inline-block text-xs font-bold text-orange-600 hover:text-orange-700 dark:text-orange-500 uppercase tracking-wider">
            ...
            <div x-show="tab === 'subir'" x-transition.opacity>
                <form action="{{ route('admin.database.restore.upload') }}" method="POST" enctype="multipart/form-data" onsubmit="return confirm('⚠️ ¿ESTÁS SEGURO?\n\nEsto reemplazará la base de datos actual.');">
                    @csrf
                    <div class="mb-4">
                        <label for="sql_file" class="block text-xs font-bold text-zinc-500 uppercase mb-2">Selecciona tu archivo .sql</label>
                        <input type="file" id="sql_file" name="sql_file" accept=".sql" required class="w-full text-sm text-zinc-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-bold file:bg-orange-50 file:text-orange-700 hover:file:bg-orange-100 dark:file:bg-orange-500/10 dark:file:text-orange-500">
                    </div>
